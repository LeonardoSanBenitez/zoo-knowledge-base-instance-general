...
triggers: my knowledge of this experiment tracker is from its 2.x era;
          mlruns, MLFLOW_ALLOW_FILE_STORE, enable_mlserver, serialization_format, ...
...
| `mlflow-contribution-gates.md` | Why an outside pull request lives or dies here, before any technical judgement: the `ready`-label auto-close bot and the **2026-03-10 cutoff** that exempts older issues (so an old, acknowledged, unclaimed bug is the only uncontested target), the PR-template bot, the DCO and `Co-Authored-By` requirements on every commit, and the `ISSUE_POLICY.md` bar a bug report must clear -- including the clause that puts a stack trace ending in a third-party library out of scope. |
